...
class DexRight(Robot):
    def __init__(self, cfg: DexConfig):
        self.env = cfg.env
        self.app = cfg.app
        self._name = "DexRight" if cfg.name is None else cfg.name
        self._prim_path = "/World/DexRight" if cfg.prim_path is None else cfg.prim_path

        self.asset_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../Assets/Robots/dexright.usd")
        ...
        self.ee = XFormPrim(self._prim_path + "/tool0", "end_effector")

    def initialize(self, physics_sim_view):
        super().initialize(physics_sim_view)

        hand = self.env.stage.GetPrimAtPath(f"{self._prim_path}/shadowhand")
        for link_prim in hand.GetChildren():
            if link_prim.HasAPI(PhysxSchema.PhysxRigidBodyAPI):
                rb = PhysxSchema.PhysxRigidBodyAPI.Get(self.env.stage, link_prim.GetPrimPath())
                rb.GetRetainAccelerationsAttr().Set(True)
            
        self.arm_dof_names = ["shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint", "wrist_1_joint", "wrist_2_joint", "wrist_3_joint",]
        self.hand_dof_names = ["WRJ2", "WRJ1", "FFJ4", "FFJ3", "FFJ2", "FFJ1", "MFJ4", "MFJ3", "MFJ2", 
            "MFJ1", "RFJ4", "RFJ3", "RFJ2", "RFJ1", "LFJ5", "LFJ4", "LFJ3", "LFJ2", "LFJ1", 
        ...
        for joint_name, config in joints_config.items():
            set_drive(f"{self._prim_path}/shadowhand/{joint_name}", "angular", "position", 0, config["stiffness"], 
                      config["damping"], config["max_force"])
