...
from pcd_utils import get_visible_indices, normalize_pcd_points
...
def get_rate_skeleton_points(part_points, ratio, sample_num):
    ...
    x_values = part_points[:, 0]
    min_x = np.min(x_values)
    max_x = np.max(x_values)

    x_cut = min_x + ratio * (max_x - min_x)
    distances_to_plane = np.abs(part_points[:, 0] - x_cut)
    close_points_mask = distances_to_plane <= 0.01
    close_points = part_points[close_points_mask]
    sorted_points = close_points[close_points[:, 1].argsort()]
    indices = np.linspace(0, len(sorted_points) - 1, sample_num).astype(int)
...
def get_part_skeleton_points(part_points):
    ...
    for ratio in ratios:
        ...
        skeleton_points = get_rate_skeleton_points(part_points, ratio, 5)
        ans.extend(skeleton_points)
        
    return ans
    

def get_scarf_mesh_skeleton_id(mesh_points):
...
def visualize_skeleton(mesh_points):
    ...
    skeleton_indices = get_scarf_mesh_skeleton_id(mesh_points)
...
if __name__ == "__main__":
    ...
    mesh_points, *_ = normalize_pcd_points(mesh_points)
