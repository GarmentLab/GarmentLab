...
from unigarment.skeleton.predict import get_skeleton
...
class LiftGarment_Collect_Deformation(BaseEnv):
    def __init__(self, garment_path, garment_position, garment_orientation):
        ...
        #     self.world,
        #     dexleft_pos=np.array([-0.75, 0.0, 0.5]),
        #     dexleft_ori=np.array([1.0, 0.0, 0.0, 0.0]),
        #     dexright_pos=np.array([0.75, 0.0, 0.5]),
        #     dexright_ori=np.array([1.0, 0.0, 0.0, 0.0]),
        # )
...
def manipulate_garment(env, save_idx, save_npz_dir, save_rgb_dir):
    ...
    min_y_points = initial_pc_points[y_values == min_y]
    left_bottom_point = min_y_points[np.argmin(min_y_points[:, 0])]  # x 最小的点
    right_bottom_point = min_y_points[np.argmax(min_y_points[:, 0])]  # x 最大的点

    x_values = initial_pc_points[:, 0]
    left_sleeve_point = initial_pc_points[np.argmin(x_values)]  # x 最小点
    right_sleeve_point = initial_pc_points[np.argmax(x_values)]  # x 最大点

    # 合并候选点
    candidates = np.vstack([left_sleeve_point, right_sleeve_point, left_bottom_point, right_bottom_point])
    ...
    for i in range(1, len(mani_trajectory)):
        env.attach.set_block_position_slowly(mani_trajectory[i])
        # for _ in range(50):
        #     env.step()
...
def collect_data(type, idx, garment_usd_path):
    ...
    env = LiftGarment_Collect_Deformation(
        ...
        garment_position=np.array([0, 0.0, 0.2]),
    ...
    # normalized_points, quaternion, scale, centeroid = normalize_pcd_points(original_points)  # 标准化后的点云用于获取骨架以及保存npz文件
    # normalized_pc_key_points, normalized_pc_key_points_id = get_skeleton(normalized_points)
    # pc_key_points = unnormalize_pcd_points(normalized_pc_key_points, scale, centeroid, quaternion) 
    
    # 保存最初的npz, rgb
    # np.savez(os.path.join(save_npz_dir, "p_0.npz"),
    #           keypoints=normalized_pc_key_points, 
    #          keypoints_id=normalized_pc_key_points_id, 
    ...
    save_npz_dir = f"data/with_sleeves/cd_original/mesh_pcd/{idx}_{garment_category}"
    os.makedirs(save_npz_dir, exist_ok=True)
    save_rgb_dir = f"data/with_sleeves/cd_original/cd_rgb_view/{idx}_{garment_category}"
    ...
    # garment_mesh_points = env.garment.get_vertice_positions()
    # mesh_keypoints, mesh_keypoints_id = get_pcd2mesh_correspondence(garment_mesh_points, pc_key_points)
...
if __name__=="__main__":
    ...
    # type = "dress"
    # idx = 0
    # garment_usd_path = "Assets/Garment/Dress/Long_LongSleeve/DLLS_Dress289/DLLS_Dress289_obj.usd"
